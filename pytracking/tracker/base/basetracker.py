...
from pytracking.tracker.config import cfg
import cv2
import numpy as np
import torch
    ...
    def get_subwindow(self, im, pos, model_sz, original_sz, avg_chans):
        ...
        if any([top_pad, bottom_pad, left_pad, right_pad]):
            ...
            if avg_chans is None:
                ...
                return torch.from_numpy(im_patch).cuda()
        ...
        if not np.array_equal(model_sz, original_sz):
            im_patch = cv2.resize(im_patch, (model_sz, model_sz))
            # if 1:  # im_patch.shape[-1] > 1:
            #     im_patch = cv2.resize(im_patch, (model_sz, model_sz))
            # else:
            #     im_patch = cv2.resize(im_patch.numpy(), (model_sz, model_sz))
            #     im_patch = torch.from_numpy(im_patch.astype(np.float32))
            #     return im_patch.cuda()
        if len(im_patch.shape) == 2:
            return torch.from_numpy(im_patch.astype(np.float32())).cuda()
