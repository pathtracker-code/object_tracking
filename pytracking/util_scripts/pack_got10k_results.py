...
def pack_got10k_pysot(tracker_name, param_name, results_path, output_name):
    ...
    for i in range(1,181):
        ...
        for run_id in range(1):
            # res = np.loadtxt('{}/{}/{}_{:03d}/{}.txt'.format(results_path, tracker_name, param_name, run_id, seq_name), dtype=np.float64)
            res = pd.read_csv('{}/{}/{}_001.txt'.format(results_path, seq_name, seq_name), dtype=np.float64, header=None).to_numpy()
            times = pd.read_csv('{}/{}/{}_time.txt'.format(results_path, seq_name, seq_name), dtype=np.float64, header=None).to_numpy()
            # times = np.loadtxt(
            #     '{}/{}/{}_{:03d}/{}_time.txt'.format(results_path, tracker_name, param_name, run_id, seq_name),
            #     dtype=np.float64)
