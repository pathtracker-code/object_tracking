...
import ltr.models.tracking.transt_circuit as transt_models
...
def run(settings):
    ...
    settings.batch_size = 16  # 38
    settings.num_workers = min(settings.batch_size, 8)
    settings.multi_gpu = False  # True
    ...
    settings.sequence_length = 16
    settings.search_gap = 8
    ...
    dataset_train = sampler.TransTSampler([got10k_train], [1], samples_per_epoch=1000*settings.batch_size, max_gap=100, processing=data_processing_train, num_search_frames=settings.sequence_length)
    ...
    param_dicts = [
        ...
        {
            "params": [p for n, p in model.named_parameters() if "circuit" in n or "mix" in n or "new" in n],
            "lr": 1e-3,
        },
    ]
