...
def run(settings):
    ...
    if debug:
        ...
        settings.batch_size = 24  # 8  # 4  # 120  # 70 # 38
        settings.num_workers = 24  # 24  # 30  # 10  # 35  # 30 min(settings.batch_size, 16)
    ...
    settings.sequence_length = 8  # 20  # 64 NEXT  # Same as PT
    ...
    param_dicts = [
        {
            "params": [p for n, p in model.named_parameters() if "circuit" in n or "rnn" in n or "trans_" in n],  # PREVIOUSLY THIS WAS "trans"
            "lr": 1e-4
        },
        # This was enabled for the paper. Disabling as a test.
        {
            "params": [p for n, p in model.named_parameters() if  "class_embed" in n or "bbox_embed" in n or "encoder" in n],
            "lr": 1e-6
        },
    ]
    print("Higher lr:")
    print([n for n, p in model.named_parameters() if "circuit" in n or "rnn" in n or "trans_" in n])  # if "encoder" not in n and "input_proj" not in n and "decoder" not in n and "backbone" not in n and "embed" not in n])
    print("*" * 60)
