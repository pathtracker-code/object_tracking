...
class FeatureFusionNetwork(nn.Module):
    ...
    def forward(self, src_temp, mask_temp, src_search, mask_search, pos_temp, pos_search):
        ...
        memory_temp, memory_search = self.encoder(src1=src_temp, src2=src_search, im_num=self.im_num,
...
class Encoder(nn.Module):
    ...
    def forward(self, src1, src2, im_num,
        ...
        for idx, layer in enumerate(self.layers):
            output1, output2 = layer(output1, output2, idx=idx, im_num=im_num, src1_mask=src1_mask,
                                     src2_mask=src2_mask,
...
class FeatureFusionLayer(nn.Module):
    ...
    def forward_post(self, src1, src2, idx, im_num,
                     src1_mask: Optional[Tensor] = None,
                     src2_mask: Optional[Tensor] = None,
                     src1_key_padding_mask: Optional[Tensor] = None,
                     src2_key_padding_mask: Optional[Tensor] = None,
        ...
        if 0:  # idx == 3 and im_num >=52:
            it_src2 = (src2.view(32, 32, 256) ** 2).mean(-1).detach().cpu()
            it_src22 = (src22.view(32, 32, 256) ** 2).mean(-1).detach().cpu()
            it_k2 = (k2.view(32, 32, 256) ** 2).mean(-1).detach().cpu()
            it_q2 = (q2.view(32, 32, 256) ** 2).mean(-1).detach().cpu()
            f = plt.figure()
            plt.subplot(1,5,1)
            plt.imshow(it_src2)
            plt.axis("off")
            ...
            plt.axis("off")
            plt.title("Key")
            plt.suptitle("Layer {} IM num {}".format(idx, im_num))
            plt.subplot(155)
            plt.axis("off")
            plt.title("Position")
            plt.imshow((pos_src2.view(32, 32, 256) ** 2).mean(-1).detach().cpu())
            plt.show()
            plt.close(f)
    ...
    def forward(self, src1, src2, idx, im_num,
                src1_mask: Optional[Tensor] = None,
                src2_mask: Optional[Tensor] = None,
                src1_key_padding_mask: Optional[Tensor] = None,
                src2_key_padding_mask: Optional[Tensor] = None,
        ...
        return self.forward_post(src1, src2, idx, im_num, src1_mask, src2_mask,
