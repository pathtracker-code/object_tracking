...
def gaussian_label_function(target_bb, sigma_factor, kernel_sz, feat_sz, image_sz, end_pad_if_even=True, density=False,
    ...
    target_center_norm = target_center_norm.to(target_bb.device) + 0.5
    center = feat_sz * target_center_norm * \
             torch.Tensor([(kernel_sz[0] + 1) % 2, (kernel_sz[1] + 1) % 2]).to(target_bb.device)
    # center = feat_sz * target_center_norm + 0.5 * \
    #                      torch.Tensor([(kernel_sz[0] + 1) % 2, (kernel_sz[1] + 1) % 2]).to(target_bb.device)

    sigma = sigma_factor * feat_sz.prod().sqrt().item()
    # sigma = torch.Tensor(sigma).to(target_bb.device)

    if end_pad_if_even:
        end_pad = (int(kernel_sz[0] % 2 == 0), int(kernel_sz[1] % 2 == 0))
    else:
        end_pad = (0, 0)
    # end_pad = end_pad.to(target_bb.device)
